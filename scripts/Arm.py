...
class Arm3Link:
    ...
    def __init__(self, q=None, q0=None, L=None):
        ...
        self.q = [math.pi / 4, math.pi * 3/4, 0] if q is None else q
        # some default arm positions
        # self.q0 = np.array([math.pi/4, math.pi/4, 0]) if q0 is None else q0
        self.q0 = [math.pi / 4, math.pi * 3/4, 0] if q0 is None else q0
        # arm segment lengths
        ...
        self.min_angles = [math.pi/8, math.pi/8, -math.pi/4]
...
def test():
    ...
    arm = Arm3Link(L=np.array([140, 152, 0]))

    # set of desired (x,y) hand positions
    x = np.arange(300 - 320, 350 - 320, 5)
    y = np.arange(135, 200, 5)
